          ...
          {filteredQuestions.length > 0 ? (
            <div className="grid grid-cols-1 gap-4">
              {filteredQuestions.map((q, i) => {
                // ✅ answer field fallback: q.answer || q.correctAnswer
                const correctAns = (q.answer || q.correctAnswer || '').trim();
                ...
                return (
                  <div key={q.id} className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm hover:shadow-md transition-all">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center space-x-2">
                        <span className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-tighter ${
                          q.type === QuestionType.MCQ       ? 'bg-indigo-100 text-indigo-700' :
                          q.type === QuestionType.TRUE_FALSE ? 'bg-orange-100 text-orange-700' :
                    ...
                    {((q.options && q.options.length > 0) || q.type === QuestionType.TRUE_FALSE) && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
                        {(q.type === QuestionType.TRUE_FALSE
                          ...
                          // ✅ compare against correctAns (answer || correctAnswer)
                          const isCorrect = opt.trim().toLowerCase() === correctAns.toLowerCase();
                          return (
                            <div
                              key={idx}
                    ...
                    {/* ✅ Correct Answer — always shown, always uses correctAns */}

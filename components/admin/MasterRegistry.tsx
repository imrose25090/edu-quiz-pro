...
const MasterRegistry: React.FC<MasterRegistryProps> = ({
  ...
  // ✅ ফিল্টার লজিক: মেমোরাইজ করা হয়েছে যাতে ডিপেন্ডেন্সি চেঞ্জ হলে ডাটা আপডেট হয়
  ...
  }, [activeTab, classes, subjects, chapters, questions, teachers, students, selectedClassId, selectedSubjectId]);
  ...
  return (
    <div className="lg:col-span-7 bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[850px] font-['Hind_Siliguri']">
      ...
      {/* ১. রিপোর্ট সামারি সেকশন - রিয়েল টাইম ডাটা */}
      <div className="p-8 bg-gradient-to-br from-slate-900 to-slate-800 text-white">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4">
          ...
          <div className="flex gap-2">
             <div className="bg-white/10 p-3 rounded-2xl border border-white/10 text-center min-w-[100px] backdrop-blur-sm">
                <div className="text-[9px] font-black text-slate-400 uppercase">Growth Rate</div>
                <div className="text-xl font-black text-emerald-400">
                    {students.length > 0 ? `+${Math.floor(students.length * 0.5)}%` : '0%'}
                </div>
             </div>
             <div className="bg-white/10 p-3 rounded-2xl border border-white/10 text-center min-w-[100px] backdrop-blur-sm">
                <div className="text-[9px] font-black text-slate-400 uppercase">Total Users</div>
                <div className="text-xl font-black text-indigo-400">{students.length + teachers.length}</div>
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-700/50 p-4 rounded-[24px] border border-slate-600 group hover:scale-105 transition-transform">
              <div className="text-[9px] font-black uppercase text-slate-400">Classes</div>
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-4 bg-slate-50/30 scroll-smooth custom-scrollbar">
        ...
        {/* TEACHERS LIST */}
        {(activeTab === 'TEACHERS' || activeTab === 'REGISTRY') && teachers.map((tchr) => (
        ...
        {/* STUDENTS LIST */}
        {(activeTab === 'STUDENT' || activeTab === 'REGISTRY') && students.map((user) => (
          <div key={user.id} className={`p-5 rounded-[28px] border bg-white flex flex-col md:flex-row justify-between items-center group transition-all shadow-sm ${user.isFrozen ? 'border-rose-200 bg-rose-50/30' : 'border-slate-100 hover:border-emerald-400 hover:shadow-md'}`}>
            <div className="flex items-center gap-5 w-full md:w-auto">
               ...
               <div>
                 ...
                 <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">Phone: {user.phone}</div>
               </div>
            </div>
        ...
        {currentData.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 opacity-30">
            ...
            <p className="font-black uppercase tracking-[0.4em] text-[10px] text-slate-900">Synchronizing Data Registry...</p>
          </div>
        )}
      </div>
    </div>

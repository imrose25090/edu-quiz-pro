  ...
  if (done) return (
    <div className="text-center py-6">
      ...
      <p className="text-sm font-bold text-slate-400 mt-1">
        {score >= 4 ? 'অসাধারণ!' : score >= 2 ? 'ভালো চেষ্টা!' : 'আরও অনুশীলন করো!'}
      </p>
  ...
  return (
    <div className="py-2">
      ...
      <div className="flex justify-center gap-2 mb-5">
        {[0,1,2,3].map(i => (
          <span key={i} className="text-3xl transition-all">{i < pieces ? pieceFills[i] : emptyPiece}</span>
        ))}
      </div>
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 font-['Hind_Siliguri']">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleAction} className="space-y-3">
          <input
            type="text" placeholder="তোমার পুরো নাম" required
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F8] font-['Hind_Siliguri']">
      ...
      <div className="max-w-5xl mx-auto px-4 pt-6 pb-20 space-y-4">
        ...
        {/* ══ HERO — 4 card grid (like the screenshot) ════ */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          {/* Card 2 — Total Points (indigo, highlighted) */}
          ...
          <div className="bg-white rounded-3xl p-6 flex flex-col justify-center shadow-sm border border-slate-100">
            ...
            <p className="text-indigo-600 font-black text-5xl leading-none">
                ...
                <div className="flex items-center gap-2">
                  {activeGame && (
                    <button onClick={() => setActiveGame(null)}
                      className="w-8 h-8 rounded-xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center font-black text-slate-500 transition-all mr-1">
                      ←
                    </button>
                  )}
                  <span className="text-xl">🎮</span>
                  <h3 className="font-black text-slate-800 text-base uppercase tracking-tight">
                    {activeGame || 'Quiz Games'}
                  </h3>
                </div>
        ...
        <div className="bg-white border border-slate-100 rounded-[24px] shadow-sm overflow-hidden">
          <button onClick={() => setShowBadges(!showBadges)}
            ...
            <svg className={`w-4 h-4 text-slate-300 transition-transform ${showBadges ? 'rotate-180' : ''}`}
              ...
              {nextBadge && (
                <div className="mb-3 flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2">
                  ...
                  <p className="text-sm font-bold text-slate-400">
                    পরের ব্যাজ: <span className="text-slate-600 font-black">{nextBadge.label}</span>
                  </p>
                </div>
              )}
              <div className="grid grid-cols-4 gap-2">
                  ...
                  return (
                    <div key={b.id} title={b.label}
                      className={`flex flex-col items-center p-3 rounded-2xl border transition-all ${
                        has ? 'bg-slate-50 border-slate-200' : 'bg-white border-slate-100 opacity-25 grayscale'
                      }`}>
                      <span className="text-2xl">{b.icon}</span>
                      <span className={`text-xs font-black mt-1.5 text-center leading-tight ${
                        has ? 'text-slate-600' : 'text-slate-400'
                      }`}>{b.label}</span>
                    </div>
                  );
                })}
        ...
        <div className="bg-white border border-slate-100 rounded-[24px] shadow-sm overflow-hidden">
          <button onClick={() => setShowBoard(!showBoard)}
            ...
            <svg className={`w-4 h-4 text-slate-300 transition-transform ${showBoard ? 'rotate-180' : ''}`}
              fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
            </svg>
          </button>
            ...
            {allStudents.slice(0, 3).map((s, i) => {
              ...
              return (
                <div key={i} className={`rounded-2xl p-3 text-center border transition-all ${
                  isMe ? 'bg-indigo-50 border-indigo-200' : 'bg-slate-50 border-slate-100'
                }`}>
                  <p className="text-xl">{rankIcon(i + 1)}</p>
                  <p className={`text-sm font-black truncate mt-1 ${isMe ? 'text-indigo-600' : 'text-slate-700'}`}>
                    {s.name}
                  </p>
                  <p className={`text-sm font-bold mt-0.5 ${isMe ? 'text-indigo-500' : 'text-slate-400'}`}>
                    {s.pts.toFixed(1)}
                  </p>
                </div>
              );
                ...
                return (
                  <div key={i} className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${
                    ...
                    <span className={`flex-1 font-black text-base truncate ${isMe ? 'text-indigo-600' : 'text-slate-700'}`}>
                      {s.name}
                      {isMe && <span className="ml-1.5 text-xs bg-indigo-600 text-white px-2 py-0.5 rounded-full">তুমি</span>}
                    </span>
                    <span className={`font-black text-base ${isMe ? 'text-indigo-500' : 'text-slate-400'}`}>
                      {s.pts.toFixed(1)}
                    </span>
                  </div>
                );
        ...
        <div className="bg-white border border-slate-100 rounded-[24px] shadow-sm overflow-hidden">
          <button onClick={() => setShowHistory(!showHistory)}
            ...
            <svg className={`w-4 h-4 text-slate-300 transition-transform ${showHistory ? 'rotate-180' : ''}`}
              fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          ...
          {showHistory && (
            <div className="px-4 pb-4 space-y-2 max-h-80 overflow-y-auto">
              {history.length === 0 && (
                <p className="text-center text-slate-300 font-bold text-base py-6">
                ...
                return (
                  <div key={idx}
                    ...
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 font-black text-sm ${
                      ...
                      pct >= 40 ? 'bg-amber-50 text-amber-500'   :
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs font-bold text-slate-400 mt-0.5">
                        {item.quizCode} · {new Date(item.date).toLocaleDateString('bn-BD')}
                      </p>
                    </div>
